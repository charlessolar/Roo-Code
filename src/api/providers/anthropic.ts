	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		while (true) {
			try {
				...
				// Check if error is a "Premature close" error
				const errorMessage = error instanceof Error ? error.message : String(error)
				if (errorMessage.includes("Premature close") && retryCount < MAX_RETRIES) {
					// Increment retry count and try again after a delay
					retryCount++
					console.warn(
						`Anthropic API encountered "Premature close" error. Retrying (${retryCount}/${MAX_RETRIES})...`,
					)
				...
				// Either it's not a "Premature close" error or we've exceeded max retries
				throw error
			}
		}
	}
